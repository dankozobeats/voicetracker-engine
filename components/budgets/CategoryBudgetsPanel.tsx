        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex gap-1 rounded-lg bg-slate-100 p-1">
            <button
              ...
              className={`rounded px-3 py-1.5 text-xs font-medium transition ${filterBy === 'all'
            ...
            <button
              ...
              className={`rounded px-3 py-1.5 text-xs font-medium transition ${filterBy === 'ok'
            ...
            <button
              ...
              className={`rounded px-3 py-1.5 text-xs font-medium transition ${filterBy === 'warning'
            ...
            <button
              ...
              className={`rounded px-3 py-1.5 text-xs font-medium transition ${filterBy === 'exceeded'
          ...
          <div className="ml-auto flex gap-1 rounded-lg bg-slate-100 p-1">
            <button
              ...
              className={`rounded px-3 py-1.5 text-xs font-medium transition ${viewMode === 'cards'
                  ? 'bg-white text-slate-900 shadow-sm'
                  : 'text-slate-600 hover:text-slate-900'
                }`}
              title="Vue cartes"
            ...
            <button
              ...
              className={`rounded px-3 py-1.5 text-xs font-medium transition ${viewMode === 'compact'
                  ? 'bg-white text-slate-900 shadow-sm'
                  : 'text-slate-600 hover:text-slate-900'
                }`}
              title="Vue compacte"
...
function CompactBudgetList({ budgets }: { budgets: CategoryBudgetResult[] }) {
  ...
  return (
    <div className="overflow-x-auto rounded-[24px] border-2 border-slate-100 shadow-xl shadow-slate-200/40">
      <table className="min-w-full divide-y divide-slate-100 bg-white">
        <thead className="bg-slate-50/50">
          <tr>
            <th className="px-6 py-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">Catégorie</th>
            <th className="px-6 py-4 text-right text-[10px] font-black uppercase tracking-widest text-slate-400">Budget</th>
            <th className="px-6 py-4 text-right text-[10px] font-black uppercase tracking-widest text-slate-400">Charges fixes</th>
            <th className="px-6 py-4 text-right text-[10px] font-black uppercase tracking-widest text-slate-400">Variables</th>
            <th className="px-6 py-4 text-right text-[10px] font-black uppercase tracking-widest text-slate-400">Total</th>
            <th className="px-6 py-4 text-right text-[10px] font-black uppercase tracking-widest text-slate-400">Restant</th>
            <th className="px-6 py-4 text-center text-[10px] font-black uppercase tracking-widest text-slate-400">Statut</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-50">
          {budgets.map((budget) => {
            ...
            return (
              <tr key={budget.category} className="hover:bg-slate-50/50 transition-colors">
                <td className="px-6 py-4 text-sm font-black text-slate-900">{budget.category}</td>
                <td className="px-6 py-4 text-sm font-bold text-slate-600 text-right">{formatCurrency(budget.budget)}</td>
                <td className="px-6 py-4 text-sm font-bold text-orange-600 text-right">{formatCurrency(budget.fixedCharges)}</td>
                <td className="px-6 py-4 text-sm font-bold text-indigo-600 text-right">{formatCurrency(budget.variableSpent)}</td>
                <td className="px-6 py-4 text-sm font-black text-slate-900 text-right">{formatCurrency(budget.spent)}</td>
                <td className={`px-6 py-4 text-sm font-black text-right ${budget.remaining < 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                  {formatCurrency(budget.remaining)}
                </td>
                <td className="px-6 py-4 text-center">
                  <span className={`inline-block rounded-lg px-2 py-1 text-[10px] font-black uppercase tracking-widest ${statusColors[budget.status]}`}>
                    {Math.round(ratio)}%
                  </span>
                </td>
              </tr>
